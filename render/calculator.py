from models import Missile, Shop, Sheet, SearchItem, ComponentData, Paint, QDrive, Weapon, Shield, PowerPlant, Cooler, MissileRack, Mount
...
class Calculator:
    ...
    def get_match_item_by_local_name(self, name: str) -> Union[ComponentData, MissileRack, Mount, None]:
        match_list: list[list[Union[ComponentData, Mount, MissileRack]]] = [
            self.missiles, self.weapons, self.bombs, self.shields, self.coolers, self.ships, self.mounts,
            self.power_plants, self.qdrives, self.qeds, self.utilities, self.missile_racks
        ]
        for component_list in match_list:
            for component in component_list:
                if component.localName == name or component.localName == name.replace("_scitem", ""):
                    return component
    ...
    def bind_ship_components(self):
        for ship in self.ships:
            for load in ship.data.loadout:
                load.mount = self.get_match_item_by_local_name(load.localName)
                if load.loadout:
                    for sub_load in load.loadout:
                        sub_load.component = self.get_match_item_by_local_name(sub_load.localName)


if __name__ == "__main__":
    calculator = Calculator()
    with open("data/ship_list.json", "w", encoding="utf-8") as f:
        ship_list = json.dumps([ship.dict() for ship in calculator.ships], ensure_ascii=False, indent=4)
        f.write(ship_list)
    a = 1

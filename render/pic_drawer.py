...
class PicDrawer:
    ...
    class ComponentDrawer:
        ...
        def add_manufacturer_chinese_name(self):
            ...
            if text.startswith("武藏"):
                text = "武藏星航"
            elif text.startswith("起源"):
                text = "起源跃动有限公司"
...
class ShipDrawer(PicDrawer.ComponentDrawer):
    ...
    def add_speed_info(self):
        if self.data.data.ifcs is None:
            return
        text = f"最大速度/加力后速度: {self.data.data.ifcs.maxSpeed}米/秒({self.data.data.ifcs.maxAfterburnSpeed}米/秒)\n"
        text += f"最大加力时间: {round(self.data.data.ifcs.afterburner.capacitorMax / self.data.data.ifcs.afterburner.capacitorAfterburnerIdleCost)}秒\n"
        text += f"加力提升: {int(self.data.data.ifcs.afterburner.afterburnAccelMultiplier.x * 100)}% / {int(self.data.data.ifcs.afterburner.afterburnAccelMultiplier.y * 100)}% / {int(self.data.data.ifcs.afterburner.afterburnAccelMultiplier.z * 100)}%\n"
        ...
        for i, turret in enumerate(turrets):
            if turret.mount is not None:
                if turret.mount.data.chineseName is not None:
                    ...
                    text = f"S{turret.mount.data.size}导弹架"
    ...
    def add_weapon_info(self):
        ...
        turret_text_list = [str(value) + key for key, value in turrets_text_dict.items()]
        for i, text in enumerate(turret_text_list):
            self.drawer.text((start_x, current_y), text + ' [炮塔]', font=self.default_font, fill="white")
            current_y += 80
